# ShippingAddressRemote

ShippingAddressRemote is a **Module Federation remote application** built with React Native and Re.Pack. It exposes a ShippingAddress module that can be dynamically loaded and consumed by the host application (`RepackApp`) at runtime.

## Overview

- **Type**: Module Federation Remote
- **Framework**: React Native 0.81.5 with Re.Pack 5.2.3
- **Purpose**: Exposes a reusable ShippingAddress component that can be consumed by other apps in the monorepo
...
│   └── module/
│       └── ShippingAddress/
│           └── index.tsx    # Exposed ShippingAddress component
...
| Command                       | Action                                   |
| :---------------------------- | :--------------------------------------- |
| `pnpm run dev:shipping`       | Metro dev server for Android (port 8084) |
| `pnpm run android`            | Run Android app (connects to port 8084)  |
| `pnpm run build:shipping:apk` | Build release APK                        |
